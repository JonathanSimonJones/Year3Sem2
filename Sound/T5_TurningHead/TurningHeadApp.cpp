...
bool Setup()
{
	...
	ok = ok && dsDevice->CreateSoundBuffer (&(theSounds[0]), "T5_SoundFiles\\woop.wav", DSBCAPS_CTRL3D);
	//ok = ok && dsDevice->CreateSoundBuffer (&(theSounds[1]), "T5_SoundFiles\\car.wav", DSBCAPS_CTRL3D);
	//ok = ok && dsDevice->CreateSoundBuffer (&(theSounds[2]), "T5_SoundFiles\\jungle.wav", DSBCAPS_CTRL3D);
	if (!ok) return false;            // sound buffer creation failed.
	soundscape[0] = new EnvSound (theSounds[0], 0, 0, 7.5);
