	...
	int numPoints)		// number of compass points.
{
	...
	device->CreateSoundBuffer (&(directions[0]), "T5_SoundFiles\\north.wav", 0);
	device->CreateSoundBuffer (&(directions[2]), "T5_SoundFiles\\east.wav", 0);
	device->CreateSoundBuffer (&(directions[4]), "T5_SoundFiles\\south.wav", 0);
	device->CreateSoundBuffer (&(directions[6]), "T5_SoundFiles\\west.wav", 0);
	if (compassPoints == 8) {
		device->CreateSoundBuffer (&(directions[1]), "T5_SoundFiles\\northeast.wav", 0);
		device->CreateSoundBuffer (&(directions[3]), "T5_SoundFiles\\southeast.wav", 0);
		device->CreateSoundBuffer (&(directions[5]), "T5_SoundFiles\\southwest.wav", 0);
		device->CreateSoundBuffer (&(directions[7]), "T5_SoundFiles\\northwest.wav", 0);
	}
	else
		for (int i=1; i<8; i+=2)
			directions[i] = NULL;
